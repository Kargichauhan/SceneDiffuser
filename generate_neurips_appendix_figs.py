...
- Uses synthetic data by default; pass your real arrays to the functions.
Usage:
  python generate_neurips_appendix_figs.py --out figs
...
def generate_e3_collision_heatmaps(
    ...
    plt.xlabel("Space"); plt.ylabel("Time"); plt.title("Collision Potential (Baseline)")
    p3a = out_dir / "Figure_E3a_baseline_heatmap"; savefig(p3a, pdf=save_pdf)

    plt.figure()
    plt.imshow(method_n, aspect='auto', origin='lower')
    plt.xlabel("Space"); plt.ylabel("Time"); plt.title("Collision Potential (Ours)")
    p3b = out_dir / "Figure_E3b_method_heatmap"; savefig(p3b, pdf=save_pdf)
...
def generate_e4_temporal_consistency(
    ...
    plt.xlabel("Time (s)"); plt.ylabel("Metric Value"); plt.title("Temporal Consistency of Validity Metrics")
    plt.legend(ncol=2, fontsize=8)
    p4 = out_dir / "Figure_E4_temporal_consistency"; savefig(p4, pdf=save_pdf)
    return p4.with_suffix(".png")

# ---------- E5: violation breakdown ----------
def generate_e5_constraint_breakdown(
    ...
    assert len(baseline_vals) == n and len(ours_vals) == n, "Values must match categories length."
    idx = np.arange(n); bar_width = 0.35
    plt.figure()
    ...
    plt.ylabel("Violation Rate"); plt.title("Constraint Violation Breakdown"); plt.legend()
    p5 = out_dir / "Figure_E5_constraint_breakdown"; savefig(p5, pdf=save_pdf)
...
def generate_e6_ablation_panels(out_dir: Path, save_pdf: bool = False) -> List[Path]:
    ...
    plt.figure(); plt.plot(x, y, label="Trajectory")
    plt.xlabel("x (m)"); plt.ylabel("y (m)"); plt.title("(a) Full Model"); plt.legend()
    p6a = out_dir / "Figure_E6a_full_model"; savefig(p6a, pdf=save_pdf); outputs.append(p6a.with_suffix(".png"))
    # (b) No collision potential
    x, y = synthetic_trajectory(seed_shift=0.5, smoothness=0.3, collision_bias=+0.6)
    plt.figure(); plt.plot(x, y, label="Trajectory")
    plt.xlabel("x (m)"); plt.ylabel("y (m)"); plt.title("(b) No Collision Potential"); plt.legend()
    p6b = out_dir / "Figure_E6b_no_collision"; savefig(p6b, pdf=save_pdf); outputs.append(p6b.with_suffix(".png"))
    # (c) No kinematic constraints
    t = np.linspace(0, 1, 200); x = t * 30.0
    y = 3*np.sin(0.6*np.pi*t) + 2*np.sin(1.0*np.pi*t + 0.2) + 0.8*np.sin(12*np.pi*t)
    plt.figure(); plt.plot(x, y, label="Trajectory")
    plt.xlabel("x (m)"); plt.ylabel("y (m)"); plt.title("(c) No Kinematic Constraints"); plt.legend()
    p6c = out_dir / "Figure_E6c_no_kinematic"; savefig(p6c, pdf=save_pdf); outputs.append(p6c.with_suffix(".png"))
    # (d) No graph attention
    x = t * 30.0
    y = 3*np.sin(0.6*np.pi*t) + 1.5*np.sin(2.2*np.pi*t) + 0.5*np.sin(8*np.pi*t) * (t > 0.35) * (t < 0.7)
    plt.figure(); plt.plot(x, y, label="Trajectory")
    plt.xlabel("x (m)"); plt.ylabel("y (m)"); plt.title("(d) No Graph Attention"); plt.legend()
    p6d = out_dir / "Figure_E6d_no_graph"; savefig(p6d, pdf=save_pdf); outputs.append(p6d.with_suffix(".png"))
    return outputs
...
def generate_e7_diversity(
    ...
    for (x, y) in trajectories:
        plt.plot(x, y, lw=1.2)
    plt.xlabel("x (m)"); plt.ylabel("y (m)"); plt.title("Diversity of Valid Trajectories (Same Initial State)")
    p7 = out_dir / "Figure_E7_diversity"; savefig(p7, pdf=save_pdf)
...
def generate_e8_energy_landscape(
    ...
    if energy_grid is None:
        xx = np.linspace(-5, 25, 250); yy = np.linspace(-10, 10, 220)
    ...
    plt.xlabel("x"); plt.ylabel("y"); plt.title("Energy Function Landscape (Lower is Better)")
    p8 = out_dir / "Figure_E8_energy_landscape"; savefig(p8, pdf=save_pdf)
...
def main():
    ...
    p3a, p3b = generate_e3_collision_heatmaps(args.out, save_pdf=args.pdf); print("E3:", p3a, p3b)
    p4 = generate_e4_temporal_consistency(args.out, save_pdf=args.pdf);     print("E4:", p4)
    p5 = generate_e5_constraint_breakdown(args.out, save_pdf=args.pdf);     print("E5:", p5)
    ab = generate_e6_ablation_panels(args.out, save_pdf=args.pdf);          print("E6:", *ab)
    p7 = generate_e7_diversity(args.out, save_pdf=args.pdf);                print("E7:", p7)
    p8 = generate_e8_energy_landscape(args.out, save_pdf=args.pdf);         print("E8:", p8)
